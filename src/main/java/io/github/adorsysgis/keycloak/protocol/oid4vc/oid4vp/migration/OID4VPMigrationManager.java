...
public final class OID4VPMigrationManager {
    ...
    /**
     * Refuses to construct the manager with duplicate migration ids: resume behavior would
     * become ambiguous (the manager picks the first match in {@link #indexAfter}) and any
     * stamped marker could refer to either copy. Failing fast at construction.
     */
    ...
    void assertOid4vpAuthFlowIsHealthy(RealmModel realm) {
        ...
        boolean exactlyOneOid4vpAuthenticator = executions.size() == 1
                && OID4VPAuthenticatorFactory.PROVIDER_ID.equals(
                        executions.getFirst().getAuthenticator());
    ...
    private boolean isRegisteredMigration(String id) {
        return migrations.stream().anyMatch(migration -> Objects.equals(migration.id(), id));
    }
}
